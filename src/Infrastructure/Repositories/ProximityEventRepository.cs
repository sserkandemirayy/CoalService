...
public class ProximityEventRepository : IProximityEventRepository
{
    ...
    public ProximityEventRepository(AppDbContext db, ICurrentUserService currentUser)
    {
        _db = db;
    ...
    private IQueryable<ProximityEvent> Scoped()
    {
        var tags = RepositoryScope.Tags(_db, _currentUser);
        return _db.ProximityEvents.Where(x => tags.Any(t => t.Id == x.TagId) || tags.Any(t => t.Id == x.PeerTagId));
    }
