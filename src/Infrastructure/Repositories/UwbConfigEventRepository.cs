...
public class UwbConfigEventRepository : IUwbConfigEventRepository
{
    ...
    public UwbConfigEventRepository(AppDbContext db, ICurrentUserService currentUser)
    {
        _db = db;
    ...
    private IQueryable<UwbConfigEvent> Scoped()
    {
        var tags = RepositoryScope.Tags(_db, _currentUser);
        return _db.UwbConfigEvents.Where(x => tags.Any(t => t.Id == x.TagId));
    }
